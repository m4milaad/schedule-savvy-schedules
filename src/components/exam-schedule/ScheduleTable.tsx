...
export const ScheduleTable = ({
  ...
  return (
    <div>
      {/* Legend */}
      <div className="px-4 py-3 border-b text-xs text-muted-foreground flex flex-wrap items-center gap-4">
        <span>Drag &amp; drop to reschedule · Max 4 exams/day · No student overlaps</span>
              ...
              {sortedDates.map(dateString => {
                ...
                return (
                  <TableRow key={dateString}>
                    ...
                    <TableCell>{date.toLocaleDateString("en-US", { weekday: "long" })}</TableCell>
                    <TableCell>{getExamTimeSlot(date)}</TableCell>
                    <TableCell>
                      <Droppable droppableId={`date-${dateString}`} direction="horizontal">
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`flex gap-2 flex-wrap min-h-[40px] p-2 rounded transition-colors ${
                                      ...
                                      <TooltipTrigger asChild>
                                        <Badge
                                          ...
                                          className={`cursor-move flex items-center gap-1 ${
                                            ...
                                            exam.semester <= 2 ? "bg-red-50 text-red-700 border-red-200" :
                                            exam.semester <= 4 ? "bg-blue-50 text-blue-700 border-blue-200" :
                                            exam.semester <= 6 ? "bg-green-50 text-green-700 border-green-200" :
                                            exam.semester <= 8 ? "bg-purple-50 text-purple-700 border-purple-200" :
